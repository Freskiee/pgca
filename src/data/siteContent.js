export const siteContent = {
  brand: {
    shortName: 'PGCA',
    fullName: 'Peregrina, Guerrero, Cardoso & Asociados',
  },

  hero: {
    title: 'Despacho Legal & Contable',
    description: 'Excelencia y experiencia a su servicio',
    primaryCta: {
      label: 'Contáctanos',
      href: '#contacto',
    },
    backgroundImage:
      'https://images.unsplash.com/photo-1511818966892-d7d671e672a2?q=80&w=1800&auto=format&fit=crop',
    sideImage:
      'https://images.unsplash.com/photo-1589994965851-a8f479c573a9?q=80&w=1200&auto=format&fit=crop',
  },

  aboutPreview: {
    title: 'Solidez profesional con visión estratégica',
    description:
      'Somos un despacho multidisciplinario con sólida trayectoria en la provisión de servicios legales y contables. Nuestro equipo está conformado por profesionistas altamente capacitados, comprometidos con ofrecer soluciones personalizadas, claras y efectivas para cada cliente.',
    highlights: [
      'Atención legal y contable integral',
      'Enfoque estratégico y profesional',
      'Relación cercana, ética y confiable',
    ],
  },

  rankings: {
    title: 'Reconocimientos y presencia destacada',
    description:
      'Estructura preparada para mostrar rankings, directorios, distintivos o instituciones donde el despacho tenga presencia y reconocimiento.',
    items: [
      { name: 'Top Ranking 2024' },
      { name: 'Top Law Firm' },
      { name: 'Law Awards Winner' },
      { name: 'Best Client Experience' },
    ],
  },

  missionVision: {
    title: 'Principios que guían nuestra práctica',
    mission: {
      title: 'Misión',
      text: 'Proporcionar asesoría legal y contable de excelencia, basada en la integridad, la ética y la profesionalidad. Nos esforzamos por ser socios estratégicos de nuestros clientes, ayudándolos a alcanzar sus objetivos y resolver sus desafíos con soluciones eficientes y efectivas.',
    },
    vision: {
      title: 'Visión',
      text: 'Ser reconocidos como líderes en el campo de la asesoría legal y contable, destacándonos por nuestra innovación, calidad de servicio y compromiso con la satisfacción del cliente. Aspiramos a crecer de manera sostenible, fortaleciendo nuestras capacidades y relaciones.',
    },
  },

  practiceAreas: {
    title: 'Áreas principales',
    description:
      'Concentramos nuestra propuesta principal en cuatro áreas estratégicas del despacho, presentadas de forma clara y estructurada.',
    items: [
      {
        title: 'Derecho Penal',
        description:
          'Asesoría y representación legal con enfoque técnico, estratégico y profesional.',
        href: '/derecho-penal',
      },
      {
        title: 'Derecho Fiscal',
        description:
          'Soluciones fiscales integrales para empresas y personas, con claridad y precisión.',
        href: '/derecho-fiscal',
      },
      {
        title: 'Derecho Corporativo',
        description:
          'Acompañamiento jurídico para decisiones empresariales, estructura y cumplimiento.',
        href: '/derecho-corporativo',
      },
      {
        title: 'Contabilidad',
        description:
          'Servicios contables y administrativos con enfoque confiable, ordenado y eficiente.',
        href: '/contabilidad',
      },
    ],
  },

  partners: {
  title: 'Nuestros socios',
  description: 'Texto breve aquí...',
  items: [
    {
        name: 'Mtro. Alfonso Yañez Peregrina',
        role: 'Especialista en Derecho Fiscal, Corporativo y Administrativo.',
        image:
            'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=900&auto=format&fit=crop',
    },
    {
        name: 'Mtro. Alejandro E. Guerrero Silva',
        role: 'Experto en Derecho Penal, negociaciones y manejo de crisis.',
        image:
            'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=900&auto=format&fit=crop',
    },
    {
        name: 'L.C., E.F. y M.D.A.F. Alberto L. Cardoso',
        role: 'Experto en Asesoría Fiscal, Contable Financiera y de Negocios.',
        image:
            'https://images.unsplash.com/photo-1560250097-0b93528c311a?q=80&w=900&auto=format&fit=crop',
    },
    {
        name: 'Mtro. Alfonso Yañez Peregrina',
        role: 'Especialista en Derecho Fiscal, Corporativo y Administrativo.',
        image:
            'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=900&auto=format&fit=crop',
    },
    {
        name: 'Mtro. Alejandro E. Guerrero Silva',
        role: 'Experto en Derecho Penal, negociaciones y manejo de crisis.',
        image:
            'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=900&auto=format&fit=crop',
    },
    {
        name: 'L.C., E.F. y M.D.A.F. Alberto L. Cardoso',
        role: 'Experto en Asesoría Fiscal, Contable Financiera y de Negocios.',
        image:
            'https://images.unsplash.com/photo-1560250097-0b93528c311a?q=80&w=900&auto=format&fit=crop',
    }
  ],
},
}